...
import { X, Download, RefreshCw, Trash2, Paperclip, Plus, FileText } from "lucide-react";
...
export function DocumentosModal({
  ...
  async function doRenovar(docId: string) {
    if (!eVenc) { setError("Indica la nueva fecha de vencimiento."); return; }
    setBusy(true); setError("");
    try {
      const arch = await fileField(editFileRef);
      const updated = await apiDocs.renovar(base, entityId, docId, { vencimiento: eVenc, ...arch });
      refresh(updated);
      setEditId(null); setEVenc("");
      if (editFileRef.current) editFileRef.current.value = "";
    } catch (err: any) { setError(err.message || "No se pudo renovar."); }
    finally { setBusy(false); }
  }
                ...
                <div key={d.id} className="rounded-lg border border-slate-200 p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                      ...
                      <button onClick={() => { setEditId(editId === d.id ? null : d.id); setEVenc(""); setError(""); }} title="Renovar" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-brand-600"><RefreshCw size={16} /></button>
                  ...
                  {editId === d.id ? (
                    <div className="mt-3 flex flex-wrap items-end gap-2 border-t border-slate-100 pt-3">
                      <div>
                        <label className="mb-1 block text-xs font-medium text-slate-500">Nueva fecha de vencimiento</label>
                        <input type="date" value={eVenc} onChange={(e) => setEVenc(e.target.value)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm outline-none focus:border-brand-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-xs font-medium text-slate-500">Reemplazar PDF (opcional)</label>
                        <input ref={editFileRef} type="file" accept="application/pdf" className="block w-52 text-xs text-slate-500 file:mr-2 file:rounded file:border-0 file:bg-slate-100 file:px-2 file:py-1 file:text-xs" />
                      </div>
                      <button disabled={busy} onClick={() => doRenovar(d.id)} className="rounded-lg bg-brand-500 px-3 py-1.5 text-sm font-semibold text-white hover:bg-brand-600 disabled:opacity-60">Guardar</button>
                    </div>
                  ) : null}
                </div>
              );
