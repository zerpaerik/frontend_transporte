...
export default function NeumaticosPage() {
  const { neumaticos, vehiculos, addNeumatico } = useData();
  ...
  const fields: Field[] = [
    ...
    { name: "marca", label: "Marca", type: "text", required: true, placeholder: "Michelin" },
    { name: "kmInstalacion", label: "Km al instalar", type: "number", default: 0 },
    { name: "kmActual", label: "Km actual", type: "number", default: 0 },
    { name: "costo", label: "Costo (S/)", type: "number", default: 0 },
    { name: "tienda", label: "Tienda", type: "text", placeholder: "Neumáticos Perú" },
    { name: "estado", label: "Estado", type: "select", options: ["Nuevo", "En uso", "Para rotar", "Reencauche", "Descartado"] },
  ];
  ...
  return (
    <div>
      ...
      <FormModal open={open} title="Registrar neumático" subtitle="Asocia la llanta a su posición exacta en la unidad." fields={fields} onSubmit={guardar} onClose={() => setOpen(false)} />
    </div>
  );
}
